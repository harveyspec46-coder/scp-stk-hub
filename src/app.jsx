...
async function fetchUserProfile(authUser) {
  const { data, error } = await supabase
    .from("users")
    .select("id, full_name, role, office, active")
    .eq("id", authUser.id)
    .single();
  ...
  return {
    ...
    role: data.role,        // "admin" | "manager" | "staff" — stored in DB
...
function AuthScreen({ onAuth }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setLErr(err.message || "Login failed. Check your email and password.");
    } finally {
      setLLoad(false);
    }
  };
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      setSErr(err.message || "Signup failed. Please try again.");
    } finally {
      setSLoad(false);
    }
  };
    ...
    <div>
      ...
      <div className="card">
        ...
        >
          {[
            { l: New job, i: "🔧", p: "crm" },
            { l: Participants, i: "🎓", p: "participants", b: 2 },
            { l: Programs, i: "🏛️", p: "programs" },
            { l: Grants, i: "💰", p: "grants" },
            { l: Votes, i: "🗳️", p: "voting", b: 2 },
            { l: Users & IDs, i: "🪪", p: "users", b: 3 },
          ].map((q, i) => (
            <button
              key={i}
              className="btn"
    ...
    <div>
      ...
      <div className="tabs">
        {[
          ...
          ["forms", "Form Templates", 5],
          ["uploads", "PDF Uploads", 5],
        ].map(([k, l, n]) => (
          <button
            key={k}
      ...
      {tab === "forms" && (
        <div>
          {[
            {
              name: "Pay It Forward Application & Agreement",
              prog: "Pay It Forward",
              fields: 18,
              subs: 12,
            },
            {
              name: "LELN Client Intake Form",
              prog: "LELN",
              fields: 14,
              subs: 9,
            },
            {
              name: "General Participant Intake",
              prog: "All programs",
              fields: 10,
              subs: 31,
            },
            {
              name: "FSURP Peer Support Agreement",
              prog: "FSURP",
              fields: 8,
              subs: 7,
            },
            {
              name: "Family Pathways Consent Form",
              prog: "Family Pathways",
              fields: 12,
              subs: 5,
            },
          ].map((f, i) => (
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: T.muted }}>
                    {f.prog} · {f.fields} fields · {f.subs} submissions
                  </div>
                </div>
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      toast("Intake link copied ✓", "success");
                    }}
                  >
                    🔗 Share
                  </button>
      ...
      {tab === "uploads" && (
        <div>
          <div
            className="upload-zone"
            onClick={() =>
              toast("File picker — upload completed physical form PDF")
            }
          >
            <div style={{ fontSize: 28, marginBottom: 7 }}>📎</div>
            <div style={{ fontSize: 13, color: T.text, marginBottom: 3 }}>
              Upload completed physical form
            </div>
            <div style={{ fontSize: 11, color: T.muted }}>
...
function ESignatures({ toast, user }) {
  ...
  const myPendingDocs = docs.filter(
    (d) =>
      d.status !== "complete" &&
      d.signers.some(
        (s) => !s.signed && (s.uid === user?.uid || s.role === user?.role)
      )
  );
